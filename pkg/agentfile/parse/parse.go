...
func ParseFile(path string) (*agentfile.Agentfile, error) { //nolint:revive // public API
	f, err := os.Open(path)
	if err != nil {
...
func Parse(r io.Reader) (*agentfile.Agentfile, error) {
...
func (p *parser) parse() (*agentfile.Agentfile, error) {
	af := &agentfile.Agentfile{Agent: newAgent()}
...
func applyInstruction(agent *agentfile.Agent, inst *instruction, heredoc string) { //nolint:funlen
	switch {
	case inst.From != nil:
	...
	case inst.Context != nil:
		ctx := &agentfile.Context{Name: inst.Context.Name}
...
func validate(af *agentfile.Agentfile) (*agentfile.Agentfile, error) {
...
func validateAgent(agent *agentfile.Agent) error {
	if agent.From == "" {
