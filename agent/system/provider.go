...
// Provider implements agent.Provider using the local filesystem.
...
// agent.Provider interface stays narrow — only the system provider
...
func (a *Provider) Load(_ context.Context) ([]agent.Agent, error) {
	...
	for _, entry := range entries {
		...
		rt, loadErr := agent.LoadRuntime(chrootfs)
		if loadErr != nil {
			continue
		}
