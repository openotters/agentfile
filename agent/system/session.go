...
func (a *Agent) ListSessionMessages(
	...
	out := make([]agent.SessionMessage, len(raw))

	for i, m := range raw {
		out[i] = agent.SessionMessage{
			Role:      m.GetRole(),
