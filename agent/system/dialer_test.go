...
import (
	...
	agentv1 "github.com/openotters/agentfile/agent/api/v1"
	"github.com/openotters/agentfile/agent/system"
)

// --- bufconn harness ----------------------------------------------------

// bufconnHarness wires a stub AgentRuntime server to an in-memory
...
func TestPrompt_NoAddrErrors(t *testing.T) {
	...
	err := a.Prompt(context.Background(), agent.PromptRequest{Prompt: "hi"}, &bytes.Buffer{})
...
func TestPromptObject_NoAddrErrors(t *testing.T) {
	...
	_, err := a.PromptObject(context.Background(), agent.ObjectPromptRequest{Prompt: "hi"})
	if err == nil || !strings.Contains(err.Error(), "has no runtime address") {
...
func TestPromptStream_DeliversEvents(t *testing.T) {
	...
	var got []agent.PromptEvent
	err := a.PromptStream(context.Background(),
		agent.PromptRequest{SessionID: "s1", Prompt: "hi"},
		func(ev agent.PromptEvent) { got = append(got, ev) },
	)
	if err != nil {
...
func TestPrompt_CapturesFinalMessage(t *testing.T) {
	...
	if err := a.Prompt(context.Background(), agent.PromptRequest{Prompt: "q"}, &buf); err != nil {
...
func TestPromptStream_ServerErrorPropagates(t *testing.T) {
	...
	err := a.PromptStream(context.Background(),
		agent.PromptRequest{Prompt: "hi"},
		func(agent.PromptEvent) { t.Fatal("no events expected on error path") },
	)
...
func TestPromptObject_SendsSchemaAndReturnsBytes(t *testing.T) {
	...
	got, err := a.PromptObject(context.Background(), agent.ObjectPromptRequest{
		Prompt:            "guess",
...
func TestPromptObject_ServerErrorWrapped(t *testing.T) {
	...
	_, err := a.PromptObject(context.Background(), agent.ObjectPromptRequest{Prompt: "x"})
