...
var noopBinPuller executor.Puller = oci.NoopPuller //nolint:gochecknoglobals // test helper
...
func TestFileExecutor_PullsBins(t *testing.T) {
	...
	if len(pulled) != 3 {
		t.Fatalf("pulled = %d, want 3 (runtime + 2 tools)", len(pulled))
	}

	if pulled[0] != "ghcr.io/openotters/runtime:latest" {
		t.Errorf("pulled[0] = %q, want runtime", pulled[0])
	}

	if pulled[1] != "ghcr.io/openotters/tools/wget:latest" {
		t.Errorf("pulled[1] = %q, want wget", pulled[1])
	}

	if _, statErr := result.FS.Stat(result.RuntimeBin); statErr != nil {
		t.Errorf("runtime binary not found: %v", statErr)
	}
